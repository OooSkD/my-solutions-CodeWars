...
└───┼───┼───┘
    ...
    * possible in sense of: the observed PIN itself and all variations considering the adjacent digits
    ...
    returns an array (or a list in Java/Kotlin and C#) of all variations for an observed PIN with a
    length of 1 to 8 digits. We could name the function getPINs (get_pins in python, GetPINs in C#).
...
public class ObservedPin {
    static void generateCombinations(Character[][] lists, List<String> result, int depth, String current) {
        if (depth == lists.length) {
            result.add(current);
            return;
        }
        for (int i = 0; i < lists[depth].length; i++) {
            generateCombinations(lists, result, depth + 1, current + lists[depth][i]);
        }
    }
    public static List<String> getPINs(String observed) {
        List<String> result = new ArrayList<>();
        // String[] variants = {"08","124","2135","326","4157", "52468","6359","748","80579","968"};
        Character[][] variants = {{'0','8'}, {'1','2','4'},
                {'2','1','3','5'}, {'3','2','6'},
                {'4','1','5','7'}, {'5','2','4','6','8'},
                {'6','3','5','9'},{'7','4','8'},
                {'8','0','5','7','9'},{'9','6','8'}};
        Character[][] needed = new Character[observed.length()][];
        for (int i = 0; i < observed.length(); i++) {
            needed[i]=variants[observed.charAt(i)-'0'];
        }
        generateCombinations(needed, result, 0, "");
        return result;
    }
}
